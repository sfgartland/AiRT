...
def getStructureForSection(transcriptSection):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": "You are a college professor structuring a transcript. Keeping the original content"},
            {"role": "user", "content": transcriptSection},
            {"role": "user", "content": "Output to markdown. Edit the previous message into paragraphs with headings. Keep the original content. Return only the last sentence of each paragraph. Wrap each sentence in square brackets."}
        ]
    )
...
def structureSection(transcriptSection, structure):
    ...
    parsEndings = [match.groups() for match in regex.finditer("(#+[^\[\]]+)?\n\[([^\[\]]+)\]", structure)]
    print(len(parsEndings))
    output = []
    for heading,parEnding in parsEndings:
        # Gets fuzzy matched sentence from body of transcript
        match = regex.search(f"({regex.escape(parEnding)})"+"{e<=2}", transcriptSection)
        if match:
            ...
            transcriptSection = transcriptSection[match.end():] # Removes processed parts of transcript
...
def getStructureFromGPT(transcriptPath):
    ...
    transcriptChunks = [transcript[i:i+chunk_size] for i in range(0, chunks, chunk_size)]
