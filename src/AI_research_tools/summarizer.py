...
def summarizeLectureTranscript(transcriptPath):
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {
                "role": "system",
                "content": "You are a college professor explaining to his advanced students.",
            },
            {"role": "user", "content": transcript},
            {
                "role": "user",
                "content": "Reply in a json object. "
...
def saveResponseToMd(response, outputPath):
    headingOrder = [
        Section("Short Summary", "short_summary"),
...
def getSummaryInOutPaths(
    inputPath, outputFolder, pattern="**/*.txt", prefix="summarized_", filetype="md"
):
